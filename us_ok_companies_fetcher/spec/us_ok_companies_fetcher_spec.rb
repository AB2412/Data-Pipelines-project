...
require_relative '../lib/us_ok_companies_fetcher'

describe UsOkCompaniesFetcher do
  context 'comprehensive match' do
    context 'should match to expected value' do
      comprehensive_check do |uid|
        input_data = UsOkCompaniesFetcher.fetch_datum(uid)
        next if input_data.nil?
        processed_data = UsOkCompaniesFetcher.process_datum(input_data)
        ...
        it 'process_datum should validate against the JSON schema' do
          ...
          expect(UsOkCompaniesFetcher.validate_datum(processed_data.except(:data))).to be_empty
        end

        it 'save_entity should persist to the sqlite' do
          expect(UsOkCompaniesFetcher.prepare_and_save_data(processed_data)).to be_empty
        end
