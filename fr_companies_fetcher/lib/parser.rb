...
module FrCompaniesFetcher
  module Parser
    ...
    def parse(fetched_datum)
      parsed_datum = {}
      parsed_datum['SIREN_DATA'] = JSON.parse(IO.read(fetched_datum['siren_data']))
      parsed_datum['SIRET_DATA'] = JSON.parse(IO.read(fetched_datum['siret_data']))
      parsed_datum['RetrievedAt'] = fetched_datum['retrieved_at']
      parsed_datum
    rescue RuntimeError
